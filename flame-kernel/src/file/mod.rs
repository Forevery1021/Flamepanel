...
impl FileService {
    ...
    pub async fn list(path: &str) -> Result<Vec<FileInfo>, AppError> {
        ...
        while let Ok(Some(entry)) = read_dir.next_entry().await {
            ...
            let (size, is_dir, permissions, modified_at) = match metadata {
                Ok(m) => {
                    (m.len() as i64, m.is_dir(), get_permissions_string(&m),
                     chrono::DateTime::<chrono::Utc>::from(m.modified().unwrap_or(std::time::SystemTime::now())).to_rfc3339())
                }
                Err(_) => (0, false, "---".into(), chrono::Utc::now().to_rfc3339()),
            };
            let mime_type = if is_dir { None } else { mime_guess::from_path(&name).first().map(|m| m.to_string()) };
            entries.push(FileInfo { name, path, size, is_dir, permissions, modified_at, mime_type });
        }
    ...
    pub async fn create_file(path: &str) -> Result<(), AppError> {
        ...
        if let Some(parent) = file_path.parent() {
            if !parent.exists() {
                return Err(AppError::BadRequest(format!("Parent directory does not exist: {}", parent.display())));
            }
        }
        if file_path.exists() {
    ...
    pub async fn rename(old_path: &str, new_path: &str) -> Result<(), AppError> {
        ...
        if new.exists() {
            return Err(AppError::BadRequest(format!("Target already exists: {}", new_path)));
        }
        if let Some(parent) = new.parent() {
            if !parent.exists() {
                return Err(AppError::BadRequest(format!("Target parent directory does not exist: {}", parent.display())));
            }
        }
    ...
    pub async fn chmod(path: &str, mode: &str) -> Result<(), AppError> {
        ...
        {
            let _ = (path, mode);
            Err(AppError::BadRequest("chmod is not supported on this platform".into()))
        }
    }
    ...
    pub async fn upload(parent_dir: &str, file_name: &str, content: &[u8]) -> Result<(), AppError> {
        ...
        if !dir.is_dir() {
            return Err(AppError::BadRequest(format!("Not a directory: {}", parent_dir)));
        }
