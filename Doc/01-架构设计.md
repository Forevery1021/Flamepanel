...
- **前端**：Vue 3 + Element Plus 单页应用，通过 REST API 与 WebSocket 与后端通信。
...
```mermaid
flowchart TD
    FE["前端<br/>Vue 3 + Element Plus + Pinia"] -->|"HTTP API /api/*"| NG["Nginx 反向代理<br/>(生产) 或 Vite Dev Proxy (开发)"]
...
- **按需加载**：`unplugin-auto-import` + `unplugin-vue-components`，Element Plus 按需引入，主包 231KB。
