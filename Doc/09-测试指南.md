...
当前基线：**162 个测试全部通过（98 集成 + 64 单元）**

| 类型 | 位置 | 数量 | 说明 |
|------|------|------|------|
| 单元测试 | 各模块 `#[cfg(test)]` | 64 | 纯逻辑测试（无需 IO/网络） |
| 集成测试 | `flame-kernel/tests/integration_test.rs` | 98 | 启动完整应用，走 HTTP 层 |
...
```mermaid
flowchart TD
    E2E["E2E（可选）<br/>前端 + 后端真实环境"] --> IT["集成测试<br/>98 个 HTTP 层"]
    IT --> UT["单元测试<br/>64 个纯逻辑"]
```
