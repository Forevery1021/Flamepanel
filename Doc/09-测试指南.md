...
当前基线：**330 个测试全部通过（168 集成 + 153 单元 + Stage/Setup/Agent 专项）**

| 类型 | 位置 | 数量 | 说明 |
|------|------|------|------|
| 单元测试 | 各模块 `#[cfg(test)]` | 153 | 纯逻辑测试（无需 IO/网络），含分页/缓存/限流/JWT/Outbox/cert/setup_service/config 等 Stage 专项 |
| 集成测试 | `flame-kernel/tests/integration_test.rs` | 153 | 启动完整应用，走 HTTP 层；含文件沙箱 symlink 逃逸 / 写目标穿越否定用例 |
| Setup 集成测试 | `flame-kernel/tests/setup_test.rs` | 13 | Setup 向导全链路（状态/两阶段初始化/无人值守/老库兼容/并发 409） |
| 多节点测试 | `flame-kernel/tests/stage5_nodes_test.rs` | 7 | Agent 心跳 Bearer 鉴权/远程命令/批量执行/注册引导令牌 |
| Agent 测试 | `agent/` | 4 | 节点 Agent 单元测试 |
