...
仓库 `docker-compose.yml` 提供两套 profile：

- **默认（生产安全）**：`flamepanel` —— **不挂载 docker.sock**，默认最小能力（`cap_drop: ALL` + 仅 `NET_BIND_SERVICE/SETUID/SETGID`），`OP_JWT_SECRET` 不写死弱默认值（留空则首次启动自动生成并持久化到 data 卷）。
- **开发便利**：`docker compose --profile dev up -d` 启动 `flamepanel-dev`（挂 `docker.sock` 只读 + 宽松能力，映射 8090 端口）。

```yaml
services:
  flamepanel:
    build: .
    container_name: flamepanel
    ports:
      - "8080:80"
    volumes:
      - flamepanel-data:/app/data
      - flamepanel-workspace:/app/workspace
    restart: unless-stopped
    environment:
      - OP_PORT=8080
      # 生产必须设置强 JWT 密钥；留空则首次启动自动生成并持久化到 data 卷
...
volumes:
  flamepanel-data:
  flamepanel-workspace:
```

> ⚠️ **危险组合警告（P0/P1）**：容器内为 **root**（nginx 需绑 80 + 可选挂载 docker.sock）。若同时满足「root 容器 + 可写 docker.sock」，容器内任意 Docker 操作等同于宿主机 root，被攻破即宿主机沦陷。**生产禁止此组合**：
> - 生产优先 **systemd + 非 root 用户** 部署（第 3 节），或使用**默认 profile（无 sock）**。
> - Docker 管理走 **Agent 白名单**（Doc/18 安全基线），不直挂 socket。
> - 需要直挂时：仅 `:ro` 只读挂载、仅本地开发（`--profile dev`）、定期审计。
> - 未挂 docker.sock 时，容器/Docker 相关 API 返回明确错误（不会静默成功）。
...
```bash
docker compose up -d             # 生产默认（无 docker.sock，最小能力）
docker compose --profile dev up -d   # 开发便利（docker.sock 只读，8090 端口）
...
# 覆盖配置启动
OP_JWT_SECRET=$JWT docker compose up -d
```
...
- [ ] Docker Compose 部署时确认**未挂载 docker.sock**（默认 profile）；挂载仅限 `--profile dev` 开发机
- [ ] Web 终端使用记录可追溯（终端会话开/关已写入操作审计日志），高危操作走定时任务审计
