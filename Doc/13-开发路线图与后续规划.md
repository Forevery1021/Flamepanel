...
**现状**：JWT 滑动刷新已落地；无默认账号（首次启动 Setup 向导 / `OP_ADMIN_PASSWORD` 无人值守初始化，新装强制首次登录改密）；Agent 注册受 Bootstrap Token 防护、心跳 Bearer 鉴权；`OP_JWT_SECRET` 默认随机生成。
...
- [x] 安装后强制修改默认密码。✅ Phase 9（无默认密码：Setup 向导创建管理员；无人值守模式强制首次登录改密）
- [x] 登录失败次数限制（Redis 不可用时内存计数 + 延迟）。✅ v0.6.0（5 次失败锁定 5 分钟，LoginAttemptStore）
- [x] Agent 节点注册引导令牌（`X-Bootstrap-Token` 常量时间校验，缺失/错误 401）。✅ Phase 9（A3.2）
...
**现状**：Agent 心跳落库与在线判定已落地（`POST /api/nodes/heartbeat/:id` Bearer 鉴权 + `last_heartbeat_at` + 前端惰性判定）。
