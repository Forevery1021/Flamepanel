...
│   ├── src/
│   │   ├── domain/        # 实体 + Repository trait（零依赖）
│   │   ├── application/   # 服务层（业务逻辑 + 编排）
│   │   ├── infrastructure/# 仓库实现（InMemory/SQLite/Docker/OS + app_store 适配器）
│   │   ├── api/           # HTTP 层（17 个 handler 模块 + 中间件 + 组合根路由）
│   │   ├── plugin/        # WASM 沙箱 + 注册表
│   │   ├── webserver/     # 5 种 Web 引擎 + 性能预设 + 原生控制（包管理/systemd）
│   │   ├── database/      # MySQL/MariaDB/Redis 原生管理
│   │   ├── firewall/      # 防火墙（ufw/firewalld/iptables）
│   │   ├── terminal/      # Web 终端（bash 子进程管道）
│   │   ├── event/         # 事件总线（broadcast + 邮件通知）
│   │   ├── resilience/    # Circuit Breaker + Retry
│   │   └── utils/         # JWT / bcrypt / 校验
│   └── tests/             # 98 集成测试 + 64 单元测试
...
- 当前版本：v0.6.0（内核），前端 0.1.0
- 测试基线：183 个测试全部通过（116 集成 + 67 单元）
- 路由规模：159 条 HTTP 路由（含 `/health` + `/api/health`）+ 3 条 WebSocket 路由
- 权限规模：64 项 RBAC 权限 + 3 角色（admin/operator/viewer）
